...
describe("VotingSystem", function () {
    ...
    beforeEach(async () => {
        [owner, user1, user2] = ethers.getSigners();
        ...
        votingSystem = await VotingSystem.deploy(voterNFT.address);
        await votingSystem.deployed();
    });

    it("Should set the VoterNFT address correctly", async () => {
        expect(await votingSystem.voterNFT()).to.equal(voterNFT.address);

    });

    it("Should NOT allow voting if user doesn't own VoterNFT", async () => {
        expect(votingSystem.connect(user2).vote(true).to.be.revertedWith("You must own a VoterNFT to vote"));
    });

    it("Should allow user to vote YES if they own VoterNFT and emit event", async () => {
        expect(votingSystem.connect(user1).vote(true)
    ...
    it("Should allow voting again after 24 hours", async () => {
        ...
        await ethers.provicer.send("evm_mine");
